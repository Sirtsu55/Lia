...
{
	std::chrono::duration<double, std::ratio<1, 1>> Duration = std::chrono::steady_clock::now() - ProgramStartTime;

	return Duration.count();
}
float GetElapsedSecondsf()
{
	std::chrono::duration<float, std::ratio<1, 1>> Duration = std::chrono::steady_clock::now() - ProgramStartTime;
